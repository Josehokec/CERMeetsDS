...
def generate_timestamps(num_events=1000, start_time=1741944117, distribution='uniform'):
    # distribution: 'exponential' | 'uniform' | 'gaussian' | 'rayleigh'
    if distribution == 'uniform':
        intervals = np.random.uniform(low=0.0, high=2.0, size=num_events)
    elif distribution == 'gaussian':
        mu=1.0
        sigma=1/3       # 3 sigma covers 99.7% data
        intervals = np.random.normal(mu, sigma, size=num_events)
        for i in range(num_events):
            if intervals[i] < 0:
                intervals[i] = 0
    elif distribution == 'rayleigh':
        mean = 1
        scale = mean / np.sqrt(np.pi / 2.0)  # mean = scale * sqrt(pi/2)
        intervals = np.random.rayleigh(scale, size=num_events)
    else:   # exponential
        lambda_rate = 1.0
...
def generate_dataset(filename):
    ...
    with open(filename, 'w') as file:
        ...
        for batch_id in range(num_batch):
            ...
            ts_list, last_time = generate_timestamps(num_events=BATCH_LEN, start_time=last_time, distribution='uniform')
