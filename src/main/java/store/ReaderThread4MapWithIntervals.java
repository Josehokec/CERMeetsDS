...
/*
this thread class is used to read data and filter records based on independent predicates and replay intervals
it is used in the extended filter-based approach to support replay intervals and avoid unnecessary reads
 */
public class ReaderThread4MapWithIntervals extends Thread{
    ...
    public ReaderThread4MapWithIntervals(File file, long start, long end, EventSchema schema, Map<String, List<Pair<IndependentPredicate, ColumnInfo>>> ipMap, ReplayIntervals replayIntervals) {
        this.file = file;
        this.start = start;
        this.end = end;
        this.schema = schema;
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            while (bytesRead < (end - start)) {
                ...
                for(int i = 0; i < recordNum; i++){
                    ...
                    // check if the record falls into any replay interval
                    long timestamp = schema.getTimestamp(bb, i * dataLen);
                    if(!replayIntervals.contains(timestamp)){
                        continue;
                    }
                    // check independent predicates
