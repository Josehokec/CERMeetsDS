...
public class ReaderThread4MapWithSWF extends Thread {
    ...
    private final long estimatedWindow;
    // query results
    List<byte[]> filteredRecords;
    Map<String, List<Integer>> varPointers;

    public ReaderThread4MapWithSWF(File file, long start, long end, EventSchema schema, Map<String,
            List<Pair<IndependentPredicate, ColumnInfo>>> ipMap, OptimizedSWF swf, long estimatedWindow) {
        this.file = file;
        this.start = start;
        this.end = end;
        this.schema = schema;
        ...
        this.swf = swf;
        this.estimatedWindow = estimatedWindow;
    }
        ...
        try (RandomAccessFile raf = new RandomAccessFile(file, "r")) {
            ...
            while (bytesRead < (end - start)) {
                ...
                for(int i = 0; i < recordNum; i++){
                    ...
                    if(!swf.query(timestamp, estimatedWindow)){
                        continue;
                    }
