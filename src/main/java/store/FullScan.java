...
public class FullScan {
    ...
    private final int cachedPageNum;
    private boolean enableS3 = false;

    public static int threadNum = 4;
    public static boolean enableFastFilter = true;

    public FullScan(String tableName) {
        this(tableName, 0);
    }

    public FullScan(String tableName, int cachedPageNum) {
        this.tableName = tableName;
        this.cachedPageNum = Math.max(cachedPageNum, 0);
        nodeId = System.getProperty("nodeId", "_0");
        //nodeId = "_0";
        // nodeId = "50M_0";
    }

    private long getUncachedStartPos(long fileSize){
        if(cachedPageNum <= 0){
            return 0;
        }
        long cachedBytes = (long) cachedPageNum * EventStore.pageSize;
        return Math.min(cachedBytes, fileSize);
    }
    ...
    public List<byte[]> scan(Map<String, List<String>> ipStringMap){
        ...
        long startPos = getUncachedStartPos(fileSize);
    ...
    public List<byte[]> concurrentScan(Map<String, List<String>> ipStringMap){
        ...
        long fileSize = store.getFileSize();
        long startPos = getUncachedStartPos(fileSize);
        long scanSize = fileSize - startPos;
        long readPageNum = scanSize / EventStore.pageSize / threadNum;
        ...
        for(int i = 0; i < threadNum; i++){
            long start = startPos + i * readPageNum * EventStore.pageSize;
            long end = i == (threadNum - 1) ? fileSize : startPos + (i + 1) * readPageNum * EventStore.pageSize;
    ...
    public EventCache scanBasedVarNameInBatch(Map<String, List<String>> ipStringMap){
        ...
        long hasReadSize = startPos;
        int curPage = (int) (startPos / EventStore.pageSize);
        int curOffset = 0;
        int recordLen = schema.getFixedRecordLen();

        if(hasReadSize >= fileSize){
    ...
    public EventCache concurrentScanBasedVarName(Map<String, List<String>> ipStringMap,
        ...
        if(enableFastFilter){
            List<ReaderThread4MapWithSWF> swfThreads = new ArrayList<>(threadNum);
            long estimatedWindow = replayIntervals.estimateWindow();
            int keyNum = replayIntervals.getKeyNumber(estimatedWindow);
            OptimizedSWF swf = new OptimizedSWF.Builder(keyNum).build();
            for(ReplayIntervals.TimeInterval interval : replayIntervals.getIntervals()) {
                swf.insert(interval.getStartTime(), interval.getEndTime(), estimatedWindow);
            }

            for(int i = 0; i < threadNum; i++){
                long start = startPos + i * readPageNum * EventStore.pageSize;
                long end = i == (threadNum - 1) ? fileSize : startPos + (i + 1) * readPageNum * EventStore.pageSize;
                ReaderThread4MapWithSWF thread = new ReaderThread4MapWithSWF(file, start, end, schema, ipMap, swf, estimatedWindow);
                swfThreads.add(thread);
                thread.start();
            }
            threads = swfThreads;
        }else{
            replayIntervals.sortAndReconstruct();
            List<ReaderThread4MapWithIntervals> intervalThreads = new ArrayList<>(threadNum);
            for(int i = 0; i < threadNum; i++){
                long start = startPos + i * readPageNum * EventStore.pageSize;
                long end = i == (threadNum - 1) ? fileSize : startPos + (i + 1) * readPageNum * EventStore.pageSize;
                ReaderThread4MapWithIntervals thread = new ReaderThread4MapWithIntervals(file, start, end, schema, ipMap, replayIntervals);
                intervalThreads.add(thread);
                thread.start();
            }
        ...
        int estimatedSize = 0;
        for(Thread thread : threads){
            if(thread instanceof ReaderThread4MapWithSWF){
                estimatedSize += ((ReaderThread4MapWithSWF) thread).getFilteredRecords().size();
            }else{
                estimatedSize += ((ReaderThread4MapWithIntervals) thread).getFilteredRecords().size();
            }
        }
        List<byte[]> filteredRecords = new ArrayList<>(estimatedSize);
        Map<String, List<Integer>> mergedVarPointers = new HashMap<>(ipStringMap.size() << 2);
        ...
        for(Thread thread : threads){
            List<byte[]> threadRecords;
            Map<String, List<Integer>> varPointers;
            if(thread instanceof ReaderThread4MapWithSWF){
                ReaderThread4MapWithSWF swfThread = (ReaderThread4MapWithSWF) thread;
                threadRecords = swfThread.getFilteredRecords();
                varPointers = swfThread.getVarPointers();
            }else{
                ReaderThread4MapWithIntervals intervalThread = (ReaderThread4MapWithIntervals) thread;
                threadRecords = intervalThread.getFilteredRecords();
                varPointers = intervalThread.getVarPointers();
            }

            int offset = filteredRecords.size();
            filteredRecords.addAll(threadRecords);
            for(Map.Entry<String, List<Integer>> entry : varPointers.entrySet()){
